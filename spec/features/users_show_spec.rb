...
RSpec.feature "UsersShows", type: :feature do
  ...
  scenario "layout of users_show" do
    ...
    expect(page).to have_link "フォロー(#{michael.following.count})",                         href: following_user_path(michael)
    expect(page).to have_link "フォロワー(#{michael.followers.count})",                       href: followers_user_path(michael)
    expect(page).to have_link "所属グループ(#{michael.joining.count})",                       href: joinings_user_path(michael)
    ...
    expect(page).to have_link "フォロー(#{forger.following.count})",                         href: following_user_path(forger)
    expect(page).to have_link "フォロワー(#{forger.followers.count})",                       href: followers_user_path(forger)
    expect(page).to have_link "所属グループ(#{forger.joining.count})",                       href: joinings_user_path(forger)
