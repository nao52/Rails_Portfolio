...
RSpec.describe User, type: :model do
  ...
  it "is invalid without name" do
    ...
    expect(@user1.errors.full_messages).to include("名前は必須項目です")
  end

  it "is invalid without email" do
    @user1.email = ""
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("メールアドレスは必須項目です")
  end

  it "is invalid without subject_id" do
    @user1.subject_id = nil
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("教科を選択してください")
  end

  it "is invalid without club_id" do
    @user1.club_id = nil
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("部活動を選択してください")
  end

  it "is invalid without kinds_of_school_id" do
    @user1.kinds_of_school_id = nil
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("校種を選択してください")
  end
  ...
  it "is invalid when name is more 51 characters" do
    @user1.name = "a" * 51
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("名前は50文字以内で入力してください")
  end
  ...
  it "is invalid when email is more 256 characters" do
    ...
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("メールアドレスは255文字以内で入力してください")
  end
  ...
  it "is invalid when profile is more 141 characters" do
    @user1.profile = "a" * 141
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("プロフィールは140文字以内で入力してください")
  end
  ...
  it "is invalid when password is less than 7 characters" do
    ...
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("パスワードは8文字以上で入力してください")
  end
  ...
  it "is invalid when password is more 17 characters" do
    ...
    expect(@user1).to_not be_valid
    expect(@user1.errors.full_messages).to include("パスワードは16文字以内で入力してください")
  end
  ...
  it "is invalid with invalid email" do
    ...
    invalid_emails.each do |invalid_email|
      ...
      expect(@user1).to_not be_valid
      expect(@user1.errors.full_messages).to include("メールアドレスの形式が不正です")
    end
  end
