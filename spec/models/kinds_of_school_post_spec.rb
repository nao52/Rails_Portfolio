...
RSpec.describe KindsOfSchoolPost, type: :model do
  ...
  let(:post) { FactoryBot.build(:kinds_of_school_post) }
  ...
  it "is first for most recent" do
    5.times do
      FactoryBot.create(:kinds_of_school_post, created_at: 2.years.ago)
    end
    most_recent_post = FactoryBot.create(:kinds_of_school_post)
    expect(most_recent_post).to eq(KindsOfSchoolPost.first)
  end

  it "is last for most old" do
    5.times do
      FactoryBot.create(:kinds_of_school_post)
    end
    most_old_post = FactoryBot.create(:kinds_of_school_post, created_at: 2.years.ago)
    expect(most_old_post).to eq(KindsOfSchoolPost.last)
  end
